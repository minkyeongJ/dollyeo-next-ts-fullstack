...
describe("roulette.utils", () => {
  describe("getRandomIndex", () => {
    ...
    it("길이가 0 이하인 경우 에러를 던져야 합니다", () => {
      expect(() => getRandomIndex(0)).toThrow("배열 길이는 0보다 커야 합니다.");
      expect(() => getRandomIndex(-1)).toThrow("배열 길이는 0보다 커야 합니다.");
    });
  });
  ...
  describe("selectRandomItem", () => {
    ...
    it("빈 배열인 경우 에러를 던져야 합니다", () => {
      expect(() => selectRandomItem([])).toThrow("배열이 비어있습니다.");
    });
  });

  describe("generateRouletteResult", () => {
    it("유효한 룰렛 결과를 반환해야 합니다", () => {
      const items = ["참가자1", "참가자2", "참가자3"];
      const result = generateRouletteResult(items);
    ...
    it("빈 배열인 경우 에러를 던져야 합니다", () => {
      expect(() => generateRouletteResult([])).toThrow(
        "룰렛 항목이 비어있습니다."
      );
    });
  });
  ...
  describe("calculateSpinDuration", () => {
    ...
    it("커스텀 값으로 시간을 계산해야 합니다", () => {
      for (let i = 0; i < 100; i++) {
        const duration = calculateSpinDuration(5000, 2000);
